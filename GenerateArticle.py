...
articles = file2.readlines() 

log = open('Tail_Article.txt','w')
aa=[] 
for article in articles: 
	i =0 

	# print("line:"+str(article))
	source=article.split('.')
	length = len(source)
	# print("len:"+str(length))

	# for i in length:
	log.writelines("article=<d><p>")
	while i<length-1:
		# print("source[i]:"+str(source[i]))
		tmp = "<s>"+str(source[i])+".</s>"
	...
	# tmp = article.replace('\n'," </s> </p> </d> ")
	# # print("source:"+str(source))
	# res = "abstract=<d> <p> <s> "+str(tmp)+"\t"+"article=<d> <p> <s> "
	# print("res:"+str(res))
	log.writelines(" </p> </d>	publisher=AFP")
	log.writelines('\n')
	# print("count:"+str(count))
	count=count+1
# log.close()
# file2 = open('article', 'r',encoding="utf-8")
# file3 = open('ThumbnailID.txt', 'r')
# log2 = open('Data.txt','w')
# articles = file2.readlines() 
